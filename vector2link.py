...
def search_program(query, top_n=3):
    ...
    csv_file_path = "radio_program_last.csv"
    ...
    # キーワード一致検索
    key_matches = data[data["content"].str.contains(query, na=False)]
    # タイトルとリンクと画像をリスト化
    key_results = [
        {"title": row["title"], "link": row["link"], "image": row["image"]} for _, row in key_matches.iterrows()]
    ...
    # 類似度上位N件を取得
    results = sorted(zip(range(len(distances)), distances), key=lambda x: x[1], reverse=True)[:top_n]

    recommended_programs = [
        {
            "title": data.iloc[idx, 1],
            "content": sentences[idx].strip(),
            "link": data.iloc[idx]["link"],  # リンクを追加
    ...
    return {"key_matches": key_results, "recommendations": recommended_programs}
